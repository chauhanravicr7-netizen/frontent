              ...
              <div className="grid grid-cols-3 gap-3">
                {deadStock.slice(0,3).map(item => (
                  <div key={item.id} className="bg-white rounded-xl p-3 border border-amber-100">
                    ...
                    <p className="text-xs text-gray-400">{item.available_quantity} {item.unit} · {yards.find(y=>y.id===item.yard_id)?.name||"—"}</p>
                    <p className="text-xs text-amber-600 font-semibold mt-1">{Math.floor((nowTs-new Date(item.last_movement_at||item.date||item.created_at).getTime())/(1000*60*60*24))} days idle</p>
...
// ── PROFIT CELL HELPER (safe - no IIFE) ──────────────────────────────────────
const ProfitCell = ({ deal, inventory }) => {
  const selInv = inventory.find(i => i.id === deal.inventory_id);
  const cost   = (selInv?.cost_price || 0) * (deal.quantity || 0);
  const profit = (deal.total_value || 0) - cost;
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  {isAdmin && <ProfitCell deal={d} inventory={inventory} />}
                </tr>
              ))}
