...
function Deals() {
  ...
  const save = async () => {
    ...
    try {
      ...
      closeDeal();
      setReceiptDeal({
        customer: custName || customers.find(c => c.id === form.customer_id)?.name || "Customer",
        product: inventory.find(i => i.id === form.product_id)?.product_name || "—",
        qty: parseFloat(form.quantity) || 0,
        price: parseFloat(form.unit_price) || 0,
        total: (parseFloat(form.quantity) || 0) * (parseFloat(form.unit_price) || 0),
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-24 md:pb-4">
      ...
      <div className="md:hidden sticky top-14 z-30 bg-white border-b border-gray-100 shadow-sm px-4 py-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => setShowAdd(true)}
            ...
            <span className="text-base">+</span> New Deal
          </button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
          {["All","Draft","Confirmed","Dispatched","Delivered","Completed"].map(t => (
      ...
      <div className="hidden md:flex px-6 pt-6 pb-3 items-center justify-between">
        ...
        <button onClick={() => setShowAdd(true)} className="bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold text-sm px-5 py-2.5 rounded-xl">+ Create Deal</button>
      </div>
      <div className="hidden md:flex gap-2 overflow-x-auto pb-1 px-6 mb-3">
        {["All","Draft","Confirmed","Dispatched","Delivered","Completed"].map(t => (
          <button key={t} onClick={() => setTab(t)} className={cls("px-4 py-1.5 rounded-full text-sm font-semibold transition-all whitespace-nowrap", tab === t ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600")}>
            {t} ({t === "All" ? deals.length : deals.filter(d => (d.status||d.stage||"").toLowerCase() === t.toLowerCase()).length})
          </button>
        ))}
      </div>
      {loading ? <Spinner /> : (
        <>
          {/* Mobile cards with swipe-to-WhatsApp */}
