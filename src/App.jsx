...
Return only the JSON array. No markdown.`,
      ...
      setCutPlans([{ plan_name: "Error: " + (e.message || "Check API key"), thickness_mm:0, width_mm:0, pieces_estimate:0, total_cft:0, waste_percent:0, best_use:"" }]);
...
Return only JSON. Be specific to timber trade. Max 4 insights.`,
      ...
      setForecast({ insights: [], summary: "Error: " + (e?.message || "Add VITE_ANTHROPIC_API_KEY to Vercel environment variables.") });
  ...
  const generateWhatsApp = async (deal) => {
    ...
    try {
      const product = deal.product_name || "Timber";
      const qty = deal.quantity || 0;
      const unit = deal.unit || "CFT";
      const rate = deal.negotiated_price || 0;
      const total = deal.total_value || (qty * rate);
      const customer = deal.customer_name || "Buyer";
      const prompt = "Write a 4-5 line professional WhatsApp message for a timber deal. Customer: " + customer + ". Product: " + product + ". Quantity: " + qty + " " + unit + ". Rate: Rs " + rate + " per " + unit + ". Total amount: Rs " + total + ". Be warm and professional. Return only the message text.";
      ...
      setWhatsappMsg("Could not generate: " + (e.message || "Check VITE_ANTHROPIC_API_KEY in Vercel settings."));
