...
const Btn = ({ children, onClick, disabled, variant = "primary", small }) => (
  <button onClick={onClick} disabled={disabled}
    className={cls(
      ...
      variant === "primary" ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-700"
    )}>
    {children}
  </button>
);
const Badge = ({ text, color = "gray" }) => {
  const c = { gray:"bg-gray-100 text-gray-600", blue:"bg-blue-100 text-blue-700", green:"bg-green-100 text-green-700", orange:"bg-orange-100 text-orange-700", red:"bg-red-100 text-red-700", purple:"bg-purple-100 text-purple-700" };
  return <span className={cls("px-2 py-0.5 rounded-full text-xs font-semibold", c[color] || c.gray)}>{text}</span>;
};
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 bg-gray-900 text-white flex items-center justify-between px-4 py-3 shadow-lg">
        ...
        <button onClick={() => setMenuOpen(p => !p)} className="text-gray-300 text-xl p-1">☰</button>
      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 flex">
        {MOBILE_NAV.map(n => (
          <NavLink key={n.to} to={n.to} end={n.to === "/"}
            className={({ isActive }) => cls(
              "flex-1 flex flex-col items-center justify-center py-2 text-xs font-semibold transition-all",
...
function Dashboard() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* ── Mobile Hero Banner ── */}
      <div className="md:hidden bg-gray-900 px-4 pt-4 pb-6">
      ...
      <div className="md:hidden sticky top-14 z-20 bg-white border-b border-gray-100 px-4 py-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => setShowAdd(true)}
            className="bg-blue-600 text-white font-bold text-sm px-4 py-2 rounded-xl flex items-center gap-1">
            + Add
          </button>
        </div>
...
function Deals() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="px-4 md:px-6 pt-4 md:pt-6 pb-2 flex flex-wrap items-center justify-between gap-3 mb-2">
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1 px-4 md:px-0">
        {TABS.map(t => (
        ...
        <>
          ...
          <div className="md:hidden space-y-3 px-4 pb-4">
            {filtered.length === 0 ? (
              ...
              <div key={d.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 px-4 py-4">
...
function AIInsights() {
  ...
  const [cutPlans, setCutPlans] = useState(null);
  const [optimizing, setOptimizing] = useState(false);

  const [forecast, setForecast] = useState(null);
  const [forecasting, setForecasting] = useState(false);

  const [whatsappDeal, setWhatsappDeal] = useState(null);
    ...
    try {
      ...
      const text = await callClaude([{
        ...
        content: [
          ...
          {
            type: "text",
            text: `You are a timber trade expert. Analyze this invoice, e-Way bill, or High Seas Sale document.
Extract timber data and return ONLY a JSON object with these exact keys (use null if not found):
{
  "product_name": "string",
  "wood_type": "string (e.g. Teak, Gurjan, Pine)",
  "thickness_mm": number,
  "width_ft": number,
  "length_ft": number,
  "girth_in": number,
...
Return only the JSON. No markdown, no explanation.`,
      ...
      setScanErr("Could not read document. Please try a clearer photo.");
    } finally {
      setScanning(false);
  ...
  const applyScanToInventory = () => {
    if (!scanResult) return;
    // Store in sessionStorage so Inventory page can pre-fill
    sessionStorage.setItem("ai_prefill", JSON.stringify(scanResult));
    window.location.hash = "/inventory";
    window.location.pathname.includes("inventory") ? window.location.reload() : (window.location.href = "/inventory");
  };

  // ── 2. AI YIELD OPTIMIZER ──────────────────────────────────────────────────
  const optimizeLog = async (item) => {
    setOptimizeItem(item); setCutPlans(null); setOptimizing(true);
    try {
      const text = await callClaude([{
        role: "user",
        content: `You are a sawmill expert for the Gandhidham timber market in India.
A round log has: Girth = ${item.girth_in || "unknown"} inches, Length = ${item.log_length_ft || item.length_ft || "unknown"} ft, Wood Type = ${item.wood_type || "Mixed Hardwood"}.
Generate exactly 3 cut plans to maximize sawn timber yield (CFT/CBM). For each plan return JSON only:
[
  {
    "plan_name": "string (e.g. Maximum Yield Plan)",
    "thickness_mm": number,
    "width_mm": number,
    "pieces_estimate": number,
    "total_cft": number,
    "waste_percent": number,
    "best_use": "string (e.g. Flooring, Furniture, Construction)"
  }
]
Return only the JSON array. No markdown.`,
      }]);
      const clean = text.replace(/```json|```/g, "").trim();
      setCutPlans(JSON.parse(clean));
    } catch {
      setCutPlans([]);
  ...
  const runForecast = async () => {
    ...
    try {
      const recentDeals = deals.slice(0, 30);
      const invSummary = inv.slice(0, 20).map(i => ({
        name: i.product_name, qty: i.available_quantity, unit: i.unit, category: i.category,
      }));
      const text = await callClaude([{
        role: "user",
        content: `You are a timber inventory forecasting expert for a trading company in Gandhidham, India.
Current inventory: ${JSON.stringify(invSummary)}
Recent deals (last 30): ${JSON.stringify(recentDeals.map(d => ({ product: d.product_name, qty: d.quantity, date: d.created_at })))}
  ...
  "insights": [
    {
      ...
      "message": "specific actionable insight in 1-2 sentences",
      "urgency": "high|medium|low"
    }
  ],
  "summary": "2-3 sentence overall business summary"
}
Return only JSON. Be specific to timber trade. Max 4 insights.`,
      }], 1200);
      const clean = text.replace(/```json|```/g, "").trim();
      setForecast(JSON.parse(clean));
    } catch {
      setForecast({ insights: [], summary: "Could not generate forecast. Please try again." });
    } finally { setForecasting(false); }
  };

  // ── 4. WHATSAPP AI QUOTE ───────────────────────────────────────────────────
  const generateWhatsApp = async (deal) => {
    setWhatsappDeal(deal); setWhatsappMsg(""); setGeneratingWA(true);
    try {
      const text = await callClaude([{
        role: "user",
        content: `Write a professional WhatsApp message for a timber deal. Use a mix of English and Hindi/Gujarati business terms naturally.
Deal details:
- Product: ${deal.product_name || "Timber"}
- Quantity: ${deal.quantity} ${deal.unit || "CFT"}
- Rate: ₹${deal.negotiated_price}/unit
- Total: ₹${deal.total_value}
- Customer: ${deal.customer_name || "Buyer"}

Write a short, professional message (4-6 lines) that:
1. Greets the customer
2. Confirms the deal details
3. Mentions the total amount
4. Asks for confirmation
Keep it warm and professional like a real Gandhidham timber trader would write.
Return only the message text, no JSON.`,
      }]);
      setWhatsappMsg(text.trim());
    } catch {
      setWhatsappMsg("Could not generate message. Please try again.");
    } finally { setGeneratingWA(false); }
  };
  ...
  const logs = inv.filter(i => i.category === "Round Log" || (i.girth_in && i.girth_in > 0));
  const urgencyColor = { high: "red", medium: "orange", low: "blue" };
  const typeIcon = { warning: "⚠️", opportunity: "📈", info: "💡" };

  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-black text-gray-800">AI Suite</h1>
        <p className="text-gray-400 text-sm">Powered by Claude AI · Built for timber trade</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-gray-900 to-blue-950 rounded-2xl p-5 text-white">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h2 className="font-black text-base">AI Bill Scanner</h2>
            <p className="text-blue-300 text-xs">Scan e-Way Bill, High Seas Sale, or any invoice</p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={scanning}
          className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold py-3 rounded-xl text-sm transition-all flex items-center justify-center gap-2">
          {scanning
            ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Reading document…</>
            : <><span>📸</span> Take Photo / Upload Document</>
          }
        </button>
        {scanErr && <p className="text-red-300 text-xs mt-2">⚠ {scanErr}</p>}
        {scanResult && (
          <div className="mt-4 bg-white/10 rounded-xl p-4">
            <p className="text-xs font-bold text-blue-300 uppercase tracking-wide mb-3">✅ AI Extracted Data</p>
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(scanResult).filter(([,v]) => v !== null).map(([k, v]) => (
                <div key={k} className="bg-white/10 rounded-lg p-2">
                  <p className="text-xs text-blue-300 capitalize">{k.replace(/_/g," ")}</p>
                  <p className="font-bold text-white text-sm">{String(v)}</p>
                </div>
              ))}
            </div>
            <button onClick={applyScanToInventory}
              className="mt-3 w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2.5 rounded-xl text-sm">
              ➕ Add to Inventory with this Data
            </button>
          </div>
        )}
      </div>

      {/* ── FEATURE 2: AI YIELD OPTIMIZER ── */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center text-xl">🪵</div>
          <div>
            <h2 className="font-black text-gray-800 text-base">AI Yield Optimizer</h2>
            <p className="text-gray-400 text-xs">Get optimal cut plans for your round logs</p>
          </div>
        </div>
        {logs.length === 0 ? (
          <p className="text-gray-400 text-sm text-center py-4">No round logs in inventory yet.</p>
        ) : (
          <div className="space-y-2">
            {logs.slice(0, 5).map(item => (
              <div key={item.id} className="flex items-center justify-between bg-green-50 rounded-xl px-4 py-3">
                <div>
                  <p className="font-semibold text-gray-800 text-sm">{item.product_name}</p>
                  <p className="text-xs text-gray-400">
                    {item.girth_in ? `Girth: ${item.girth_in}"` : ""} {item.log_length_ft ? `· Length: ${item.log_length_ft}ft` : ""}
                  </p>
                </div>
                <button
                  onClick={() => optimizeLog(item)}
                  disabled={optimizing && optimizeItem?.id === item.id}
                  className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-3 py-2 rounded-lg disabled:opacity-60">
                  {optimizing && optimizeItem?.id === item.id ? "⏳ Analyzing…" : "✨ Optimize"}
                </button>
              </div>
            ))}
          </div>
        )}
        {optimizeItem && cutPlans && (
          <div className="mt-4">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Cut Plans for {optimizeItem.product_name}</p>
            {cutPlans.length === 0
              ? <p className="text-gray-400 text-sm">Could not generate plans. Try again.</p>
              : cutPlans.map((plan, i) => (
                <div key={i} className={cls("rounded-xl p-4 mb-3 border-2", i === 0 ? "bg-green-50 border-green-300" : "bg-gray-50 border-gray-100")}>
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-black text-gray-800 text-sm">{plan.plan_name}</p>
                    {i === 0 && <Badge text="Best Yield" color="green" />}
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    <div className="bg-white rounded-lg p-2"><p className="text-gray-400">Size</p><p className="font-bold">{plan.thickness_mm}×{plan.width_mm}mm</p></div>
                    <div className="bg-white rounded-lg p-2"><p className="text-gray-400">Total CFT</p><p className="font-bold text-green-700">{plan.total_cft}</p></div>
                    <div className="bg-white rounded-lg p-2"><p className="text-gray-400">Waste</p><p className="font-bold text-orange-600">{plan.waste_percent}%</p></div>
                  </div>
                  <p className="text-xs text-gray-500 mt-2">Best for: {plan.best_use}</p>
                </div>
              ))
            }
          </div>
      ...
      {/* ── FEATURE 3: AI INVENTORY FORECAST ── */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center text-xl">🔮</div>
            <div>
              <h2 className="font-black text-gray-800 text-base">AI Forecast</h2>
              <p className="text-gray-400 text-xs">Inventory & pricing insights from your data</p>
            </div>
          </div>
          <button onClick={runForecast} disabled={forecasting}
            className="bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold px-4 py-2 rounded-xl disabled:opacity-60 flex items-center gap-1">
            {forecasting ? <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />Analyzing…</> : "▶ Run Forecast"}
          </button>
        </div>
        {!forecast && !forecasting && (
          <div className="text-center py-6">
            <p className="text-4xl mb-2">🔮</p>
            <p className="text-gray-400 text-sm">Click Run Forecast to analyze your last 30 days of sales and inventory data.</p>
          </div>
        )}
        {forecast && (
          <>
            <div className="bg-purple-50 rounded-xl p-4 mb-4">
              <p className="text-sm text-purple-800 font-medium">{forecast.summary}</p>
            </div>
            <div className="space-y-3">
              {forecast.insights.map((ins, i) => (
                <div key={i} className={cls("rounded-xl p-4 border", ins.urgency === "high" ? "bg-red-50 border-red-100" : ins.urgency === "medium" ? "bg-orange-50 border-orange-100" : "bg-blue-50 border-blue-100")}>
                  <div className="flex items-center gap-2 mb-1">
                    <span>{typeIcon[ins.type] || "💡"}</span>
                    <p className="font-bold text-gray-800 text-sm">{ins.product}</p>
                    <Badge text={ins.urgency} color={urgencyColor[ins.urgency] || "gray"} />
                  </div>
        ...
        {/* Always-visible low stock alerts */}
        {lowStock.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-2">⚠ Low Stock Alerts</p>
            <div className="space-y-2">
              {lowStock.slice(0, 4).map(i => (
                <div key={i.id} className="flex items-center justify-between bg-red-50 rounded-lg px-3 py-2">
                  <span className="text-sm text-gray-700">{i.product_name}</span>
      ...
      {/* ── FEATURE 4: WHATSAPP AI QUOTE ── */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center text-xl">💬</div>
          <div>
            <h2 className="font-black text-gray-800 text-base">AI WhatsApp Quote</h2>
            <p className="text-gray-400 text-xs">Generate professional deal messages in seconds</p>
          </div>
        </div>
        {deals.length === 0
          ? <p className="text-gray-400 text-sm text-center py-4">No deals yet. Create a deal first.</p>
          : (
            <div className="space-y-2">
              {deals.slice(0, 4).map(deal => (
                <div key={deal.id} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                  <div>
                    <p className="font-semibold text-gray-800 text-sm">{deal.customer_name || "Customer"}</p>
                    <p className="text-xs text-gray-400">{deal.product_name} · {fmt(deal.total_value)}</p>
                  </div>
                  <button
                    onClick={() => generateWhatsApp(deal)}
                    disabled={generatingWA && whatsappDeal?.id === deal.id}
                    className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-3 py-2 rounded-lg disabled:opacity-60">
                    {generatingWA && whatsappDeal?.id === deal.id ? "⏳" : "✍️ Write"}
                  </button>
                </div>
              ))}
            </div>
        ...
        {whatsappMsg && (
          <div className="mt-4 bg-green-50 border border-green-200 rounded-xl p-4">
            <p className="text-xs font-bold text-green-700 uppercase tracking-wide mb-2">Generated Message</p>
            <p className="text-sm text-gray-700 whitespace-pre-line leading-relaxed">{whatsappMsg}</p>
            <button onClick={sendWhatsApp}
              className="mt-3 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2">
