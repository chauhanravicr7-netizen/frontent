...
const SlidePanel = ({ title, open, onClose, children, wide }) => (
  <>
    ...
    <div className={cls(
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b bg-gradient-to-r from-gray-900 to-gray-800">
        <h2 className="text-base font-black text-white">{title}</h2>
        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white text-xl transition-all">×</button>
      </div>
...
function AIInsights() {
  ...
  const callClaude = async (messages, maxTokens = 1000) => {
    ...
    if (!apiKey) {
      throw new Error("Anthropic API key not configured. Add VITE_ANTHROPIC_API_KEY to Vercel environment variables.");
    }
