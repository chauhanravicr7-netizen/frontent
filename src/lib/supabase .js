...
export async function deductStockForDeal(dealId) {
  try {
    const dealResult = await sb.from("deals").select("*").eq("id", dealId).single();
    if (dealResult.error) throw dealResult.error;
    
    const deal = dealResult.data;
    if (!deal || !deal.inventory_id) {
      return { success: false, error: "No inventory linked" };
    }

    const invResult = await sb.from("inventory").select("*").eq("id", deal.inventory_id).single();
    ...
    const updateResult = await sb.from("inventory").update({
